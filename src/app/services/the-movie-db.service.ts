...
export class TheMovieDbService {
  ...
  search(query: string, filtro?: string, reset: boolean = false) {
    this.page++
    if (reset) {
      this.page = 1;
    }
    return this.httpService.get(`${this.urlBase}search/movie?api_key=${this.apiKey}&query=${query}${filtro ? `&sort_by=${filtro}`:''}`)
  }
